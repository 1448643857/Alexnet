...
model_dir = caffe_root + '/home/ch/Alexnet/'
deploy = model_dir + 'deploy.prototxt'
model_name = 'bvlc_alexnet'
caffemodel = model_dir + 'snapshot/bvlc_iter_5000.caffemodel'
test_prototxt = model_dir + 'train_val.prototxt'
solver_proto = model_dir + 'solver.prototxt'

solver = caffe.SGDSolver(solver_proto)

# 要修剪的层
prune_layers = ['fc6','fc7','fc8']
# 测试修剪率
'''
...
'''
test_threshold_list = [[0.3], [0.4], [0.5]]
# 验证修剪率
...
# 各层对应的修剪率
threshold = [0.3,0.2,0.3]
net = caffe.Net(deploy, caffemodel, caffe.TEST)
# 修剪
prune(threshold, net, prune_layers)
# 保存修剪后的稀疏网络模型
